...
export default function TaskGrid(props){
    ...
    const [open, setOpen] = useState(false) // State para abrir o Component View
    const [editTask, setEditTask] = useState(false) // State para abrir o Component Edit
    const [del, setDel] = useState(false) // State para abrir o Component Delete
    const [nameUpdate, setNameUpdate] = useState() // State que guarda o input da task, para fazer a atualização da tarefa
    const [userData, setUserData] = useState({ // State do objeto de cada tarefa
        id: '',
        name: '',
        theme: '',
        title: ''
        ...
        <Grid container>
            <Grid item xs={12} direction={'row'} >
                ...
                sx={{
                    overflowX: 'hidden', 
                    overflowY: 'scroll', 
                    '&:hover':{'&::-webkit-scrollbar-thumb':{backgroundColor: 'gray'}},
                    '&::-webkit-scrollbar':{width: '5px'}, 
                    '&::-webkit-scrollbar-thumb':{backgroundColor: 'transparent', 
                    borderRadius: '20px'}, 
                    '&::-webkit-scrollbar-track':{backgroundColor: 'trasnparent'}
                    }}>
                            ...
                            <Grid item lg={4} md={4} sm={6} xs={props.toggle <= 535 ? 12 : 6} key={itemObj.key} >
                                ...
                                >
                                    <Card sx={{backgroundColor: itemObj.theme}}>
                                        <CardHeader 
                                        title={ <Typography fontSize={24} noWrap>{itemObj.title}</Typography> }  
                                        sx={{
                                            display: 'block', 
                                            textAlign: 'center', 
